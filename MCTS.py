...
class MCTS():
	...
	def _delGarbageNodes(self,inpnode, sel_action):
		currentedges = inpnode.edges
		for action, edge in currentedges:
			if action == sel_action:
				continue
			currentNode = edge.outNode
			delID = currentNode.state.id
			if not edge.outNode.isLeaf():
				self._delGarbageNodes(currentNode, -1)
			if delID in self.tree:
				del self.tree[delID]
